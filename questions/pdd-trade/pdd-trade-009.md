...
**30 秒电梯演讲：** 不同业务场景（计数/排行/去重/对象）如何用最合适的数据结构高效处理？简单说就是——Redis 丰富数据结构支撑交易各种场景——String（缓存/计数器）、Hash（用户信息）、ZSet（排行榜）、Set（去重/标签）、Stream（消息）。

**展开框架：**
1. **String** — String：缓存、计数器（incr）、分布式锁
2. **Hash** — Hash：对象存储（用户信息）
3. **List** — List：队列、最新列表
