...
- 抢占
feynman:
  essence: 大规模定时任务分片与抢占的核心不是背概念，而是先识别业务目标、流量形态、失败模式和一致性要求，再用可观测、可回滚、可扩展的工程手段落地。
...
> 我会先确认业务目标和 SLA。对于“大规模定时任务分片与抢占”，核心是 定时任务 与 分片 的平衡。我的方案是：主链路保持简单可靠，关键状态落 MySQL 并用唯一键/版本号保证幂等；热点读用缓存但设置失效和回源保护；非核心动作走 MQ 异步，消费端做幂等和补偿；入口到下游都配置超时、限流、熔断和降级。最后通过压测、灰度、监控和回滚预案验证。
