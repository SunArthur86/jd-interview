...
可以把“类加载机制、双亲委派与 SPI 扩展”想成一个经验丰富的值班负责人：类加载 是他的工具箱，SPI 是他的调度台，插件化 是他的应急预案。平时他不抢业务主流程的方向盘，但会在流量突增、依赖抖动、数据不一致时提醒团队：先看指标，再控风险，最后再谈优化。

### 面试现场 60 秒回答

> 面试官如果问我“类加载机制、双亲委派与 SPI 扩展”，我会先确认业务规模、SLA 和一致性要求。我的设计思路是：主链路保持简单可靠，关键状态用幂等、状态机和事务边界兜住；高并发部分用限流、隔离、缓存或队列削峰；异常链路必须有补偿、对账和告警。这个题最容易翻车的是 Full GC 或并发标记跟不上导致 P99 抖动，所以我会提前设计灰度、回滚和监控，重点看 young_gc_pause_p99、old_gc_count。如果要进一步演进，我会先旁路验证，再小流量灰度，最后沉淀成团队规范或平台能力。
