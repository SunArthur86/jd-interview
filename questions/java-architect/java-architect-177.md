...
可以把“IM 消息已读未读与多端同步”想成一个经验丰富的值班负责人：IM 是他的工具箱，多端同步 是他的调度台，消息 是他的应急预案。平时他不抢业务主流程的方向盘，但会在流量突增、依赖抖动、数据不一致时提醒团队：先看指标，再控风险，最后再谈优化。

### 面试现场 60 秒回答

> 面试官如果问我“IM 消息已读未读与多端同步”，我会先确认业务规模、SLA 和一致性要求。我的设计思路是：主链路保持简单可靠，关键状态用幂等、状态机和事务边界兜住；高并发部分用限流、隔离、缓存或队列削峰；异常链路必须有补偿、对账和告警。这个题最容易翻车的是 单 AZ 或单实例成为隐性单点，所以我会提前设计灰度、回滚和监控，重点看 availability_slo、failover_seconds。如果要进一步演进，我会先旁路验证，再小流量灰度，最后沉淀成团队规范或平台能力。
