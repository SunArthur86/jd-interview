...
- 解耦
feynman:
  essence: 事件驱动架构与 Outbox 模式的核心不是背概念，而是先识别业务目标、流量形态、失败模式和一致性要求，再用可观测、可回滚、可扩展的工程手段落地。
...
> 我会先确认业务目标和 SLA。对于“事件驱动架构与 Outbox 模式”，核心是 EDA 与 Outbox 的平衡。我的方案是：主链路保持简单可靠，关键状态落 MySQL 并用唯一键/版本号保证幂等；热点读用缓存但设置失效和回源保护；非核心动作走 MQ 异步，消费端做幂等和补偿；入口到下游都配置超时、限流、熔断和降级。最后通过压测、灰度、监控和回滚预案验证。
...
可以把“事件驱动架构与 Outbox 模式”想成一个经验丰富的值班负责人：EDA 是他的工具箱，Outbox 是他的调度台，解耦 是他的应急预案。平时他不抢业务主流程的方向盘，但会在流量突增、依赖抖动、数据不一致时提醒团队：先看指标，再控风险，最后再谈优化。

### 面试现场 60 秒回答

> 面试官如果问我“事件驱动架构与 Outbox 模式”，我会先确认业务规模、SLA 和一致性要求。我的设计思路是：主链路保持简单可靠，关键状态用幂等、状态机和事务边界兜住；高并发部分用限流、隔离、缓存或队列削峰；异常链路必须有补偿、对账和告警。这个题最容易翻车的是 边界不清导致跨服务强耦合，所以我会提前设计灰度、回滚和监控，重点看 success_rate、latency_p99。如果要进一步演进，我会先旁路验证，再小流量灰度，最后沉淀成团队规范或平台能力。
