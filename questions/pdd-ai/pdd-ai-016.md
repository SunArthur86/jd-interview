...
**30 秒电梯演讲：** 混部下 Prefill 阻塞 Decode（反之亦然），SLO 和吞吐难兼顾？简单说就是——PD 分离是把 LLM 推理的 Prefill（首 token，算力瓶颈）和 Decode（续写，带宽瓶颈）拆到不同实例，各自优化硬件和调度，整体吞吐和 SLO 双提升。混部互相阻塞；分离 + KV 跨传（RDMA）。

**展开框架：**
1. **Prefill 算力瓶颈** — Prefill 算力瓶颈，Decode 带宽瓶颈
2. **混部互相阻塞** — 混部互相阻塞
3. **分离 + K** — 分离 + KV 跨传（RDMA）

**收尾：** 您想继续往深里聊吗——比如「KV 怎么传？」
