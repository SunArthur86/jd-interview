...
**30 秒电梯演讲：** 如何用任意 key 快速存取 value？简单说就是——HashMap 是"用哈希函数把 key 映射到桶数组"，O(1) 读写；中台存特征/路由表/缓存大量用 Map，必须懂扩容/树化/并发安全。负载因子 0.75，2 倍扩容；1.8 尾插 + 红黑树（防退化/死循环）。

**展开框架：**
1. **桶 + 链表** — 桶 + 链表/树（≥8 转树）
2. **负载因子 0.75** — 负载因子 0.75，2 倍扩容
3. **1.8 尾插** — 1.8 尾插 + 红黑树（防退化/死循环）

**收尾：** 您想继续往深里聊吗——比如「HashMap 链表为什么转红黑树？」
