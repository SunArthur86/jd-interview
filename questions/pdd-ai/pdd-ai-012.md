...
**30 秒电梯演讲：** 怎么在高并发下解耦服务、削峰填谷、可靠传递事件？简单说就是——Kafka 是"高吞吐分布式消息队列"，生产者写 topic，消费者订阅；中台用它做事件驱动、削峰、解耦（用户行为/特征计算/模型事件）。高吞吐：顺序写+零拷贝+批量；消费组分区分担。

**展开框架：**
1. **三角色** — 三角色：Producer/Broker/Consumer
2. **高吞吐** — 高吞吐：顺序写+零拷贝+批量
3. **消费组分区分** — 消费组分区分担

**收尾：** 您想继续往深里聊吗——比如「怎么保证消息不丢？」
