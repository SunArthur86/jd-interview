...
**30 秒电梯演讲：** 聊到volatile 原理，我的理解是——volatile 用"内存屏障"保证可见性（写立即对其他线程可见）和有序性（禁止指令重排），但不保证原子性；happens-before 是 JMM 定义的"先于"关系，决定何时能看到对方操作。打个比方，volatile 像公开黑板——老师（线程A）写完立即通知所有学生（其他线程），学生看到的是最新内容，但多人同时改黑板会乱（无原子性）。

**展开框架：**
1. **volatile 两语义** — 可见性（写刷主存、读从主存）+ 有序性（禁止重排）
2. **volatile 不保证原子性（i+** — volatile 不保证原子性（i++ 仍不安全）
3. **happens-before 8 大规则**
