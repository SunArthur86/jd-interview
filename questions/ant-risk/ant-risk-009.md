...
**30 秒电梯演讲：** 聊到MySQL 事务隔离级别，我的理解是——InnoDB 用 undo log + read view 实现 MVCC，让读写互不阻塞：每行有多个历史版本，事务按"开始时的快照"读，写通过版本链+隐藏列维护。打个比方，MVCC 像维基百科的版本历史——每篇文章（行）有多个历史版本，读者读"快照时刻的版本"，作者改出新版本不冲突。

**展开框架：**
1. **4 级隔离** — 读未提交 / 读已提交（RC）/ 可重复读（RR，InnoDB 默认）/ 串行化
2. **RR 通过 read view 在事** — RR 通过 read view 在事务首次读时建立，整个事务复用 → 可重复读
3. **RC 每次 select 都新建 read view** — 能看到最新已提交

**收尾：** 这块我在项目里也踩过坑——想深入的话，可以接着聊：为什么 InnoDB 默认 RR 而不是 RC？您更想看哪个方向？
