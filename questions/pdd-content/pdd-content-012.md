...
**30 秒电梯演讲：** 多业务模块需要事件通知，如何解耦+削峰+可靠传递？简单说就是——Kafka 用"分区+副本+消费者组"实现高吞吐解耦；评价/直播事件（发布/点赞/审核）走 Kafka 削峰+异步扩散到下游。副本：Leader+Follower；消费者组：组内分区分担。

**展开框架：**
1. **分区** — 分区：并行单元
2. **副本** — 副本：Leader+Follower
3. **消费者组** — 消费者组：组内分区分担

**收尾：** 您想继续往深里聊吗——比如「怎么保证不丢？」
