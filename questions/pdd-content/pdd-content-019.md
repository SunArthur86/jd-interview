...
**30 秒电梯演讲：** 连接数大（百万）时如何用少量线程处理？简单说就是——Java IO 演进 BIO→NIO→AIO，对应"一连接一线程→多路复用→异步回调"；直播/弹幕长连接用 Netty（NIO+Reactor）扛百万并发。NIO：Selector 多路复用；AIO：异步回调。

**展开框架：**
1. **BIO** — BIO：一连接一线程
2. **NIO** — NIO：Selector 多路复用
3. **AIO** — AIO：异步回调

**收尾：** 您想继续往深里聊吗——比如「Netty 怎么扛百万连接？」
