...
**30 秒电梯演讲：** 多核 CPU 缓存+重排导致共享变量不可见/乱序，如何约束？简单说就是——volatile 用"内存屏障"保证可见性+禁止指令重排，但不保证复合操作原子性；内容场景如直播在线人数、Feed 计数常用 volatile 做标志位。禁止重排：内存屏障；不保证原子：i++ 要 Atomic。

**展开框架：**
1. **可见性** — 可见性：刷主存+失效缓存
2. **禁止重排** — 禁止重排：内存屏障
3. **不保证原子** — 不保证原子：i++ 要 Atomic

**收尾：** 您想继续往深里聊吗——比如「volatile 和 synchronized 区别？」
