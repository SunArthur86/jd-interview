...
**30 秒电梯演讲：** 评价是购买决策核心 UGC，如何保证可信+合规+实时？简单说就是——评价系统是"用户评价+商家回复+平台审核+评分聚合+行家社区"的内容闭环；技术关键是审核流（机审+人审）、防刷、评分聚合一致性。评分：增量+定时校准；防刷：内容+行为双维。

**展开框架：**
1. **生命周期** — 生命周期：提交→机审→人审→上架
2. **评分** — 评分：增量+定时校准
3. **防刷** — 防刷：内容+行为双维

**收尾：** 您想继续往深里聊吗——比如「怎么防止刷好评？」
