...
**30 秒电梯演讲：** 软件有共性设计需求（创建/解耦/扩展），如何沉淀复用？简单说就是——设计模式是"前人总结的面向对象设计套路"；内容场景常用单例（服务）、策略（审核规则/排序）、观察者（事件分发）、模板方法（流程钩子）。结构：适配器/装饰器/代理；行为：策略/观察者/责任链。
