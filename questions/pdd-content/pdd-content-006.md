...
**30 秒电梯演讲：** 全表扫描 O(N) 慢，如何用数据结构加速查询？简单说就是——MySQL 索引用 B+ 树（多叉+叶子链表）让范围查询 O(log N)；内容场景如评价按 (商品ID,创建时间) 建联合索引，遵循最左前缀。联合索引最左前缀；覆盖索引不回表。

**展开框架：**
1. **B+ 树** — B+ 树：非叶索引+叶子数据+链表
2. **联合索引最左** — 联合索引最左前缀
3. **覆盖索引不回** — 覆盖索引不回表

**收尾：** 您想继续往深里聊吗——比如「为什么用 B+ 树不用 B 树/红黑树？」
