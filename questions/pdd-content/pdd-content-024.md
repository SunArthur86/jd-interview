...
**30 秒电梯演讲：** UGC 海量+合规风险，如何低成本+高准确+低延迟审核？简单说就是——UGC 审核架构是"接入→预处理→机审（规则+模型）→人审→回流"漏斗+事件驱动；用规则引擎+模型服务+人审台组合，平衡准确率与成本。多模态：文本/图/视频/音；引擎：Drools/自研规则。
