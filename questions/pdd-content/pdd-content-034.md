...
**30 秒电梯演讲：** LLM 推理延迟高+显存大+成本贵，如何优化？简单说就是——LLM 推理优化是"用 KV Cache+连续批处理+量化+分布式推理"降延迟提吞吐；内容场景高并发（审核/客服）需深度优化。连续批处理：动态拼请求；量化：INT8/INT4。

**展开框架：**
1. **KV Cache** — KV Cache：缓存 K/V 避免重算
2. **连续批处理** — 连续批处理：动态拼请求
3. **量化** — 量化：INT8/INT4
