...
**30 秒电梯演讲：** 内容社区突发大流量（直播开播/活动），如何扛住？简单说就是——高并发四件套（池化+多级缓存+弹性扩容+异步队列削峰）是内容社区扛流量的标配；评价写入用池+缓存挡读、直播弹幕用队列削峰。缓存：Caffeine+Redis；扩容：HPA+分片。

**展开框架：**
1. **池化** — 池化：HikariCP/线程池
2. **缓存** — 缓存：Caffeine+Redis
3. **扩容** — 扩容：HPA+分片

**收尾：** 您想继续往深里聊吗——比如「缓存击穿/穿透/雪崩？」
