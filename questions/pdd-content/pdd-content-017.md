...
**30 秒电梯演讲：** 关注关系下，如何高效分发内容到粉丝？简单说就是——Feed 流是"用户主页/关注页"的内容流，核心是"推（写扩散）vs 拉（读扩散）"的取舍；普通用户用推模式、大 V 用拉模式、混合是工业方案。拉：读扩散（适合大 V）；混合：普通推+大 V 拉。

**展开框架：**
1. **推：写扩散** — 推：写扩散（适合普通用户）
2. **拉：读扩散** — 拉：读扩散（适合大 V）
3. **混合** — 混合：普通推+大 V 拉

**收尾：** 您想继续往深里聊吗——比如「大 V 发 Feed 怎么处理？」
